IDRIS_INCLUDES += -I"$(IDRIS_SUPPORT_SRC_PATH)/refc" -I"$(IDRIS_SUPPORT_SRC_PATH)/c" -I.
...
build/exec/%.c: %.idr
	...
	sed -i 's/Value \*mainExprVal/sysi_prepare();\n   Value *mainExprVal/' "$@"
	sed -i 's|// add header(s) for library: libidris2_support|#include "sysi.h"|' "$@"
